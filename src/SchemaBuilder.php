...
final class SchemaBuilder
{
    ...
    public function __construct(
        ...
        $this->resolver = $resolveMiddleware($plugins, 0);
    }
    ...
    public function build(): Schema
    {
        ...
        $schema = SchemaExtender::extend($schema, $document, $options, function (
            array $typeConfig,
            TypeDefinitionNode $typeDefinitionNode,
            array $definitionMap
        ) {
            $name = $typeConfig['name'];
            $resolvers = $this->resolvers[$name] ?? [];

            if ($typeDefinitionNode instanceof UnionTypeDefinitionNode || $typeDefinitionNode instanceof InterfaceTypeDefinitionNode) {
                $resolveType = $resolvers[ResolverMapInterface::RESOLVE_TYPE] ?? null;
                if ($resolveType) {
                ...
                $typeConfig['serialize'] = $resolvers[ResolverMapInterface::SERIALIZE] ?? null;
                $typeConfig['parseValue'] = $resolvers[ResolverMapInterface::PARSE_VALUE] ?? null;
                $typeConfig['parseLiteral'] = $resolvers[ResolverMapInterface::PARSE_LITERAL] ?? null;
            } elseif ($typeDefinitionNode instanceof ObjectTypeDefinitionNode) {
                $typeConfig['resolveField'] = $this->resolver;
            } elseif ($typeDefinitionNode instanceof EnumTypeDefinitionNode) {
                if (is_array($resolvers)) {
                    foreach ($typeConfig['values'] as $key => &$value) {
                        $value['value'] = $resolvers[$key] ?? $key;
                    }
                } elseif (enum_exists($resolvers)) {
                    foreach ($typeConfig['values'] as $key => &$value) {
                        $value['value'] = $resolvers::from($key);
                    }
                }
            }
